...
export default function TeamMessageBoard({ user }: { user?: DemoUser | null }) {
  ...
  async function postMessage(event: FormEvent) {
    ...
    setStatus('Posting...');
    const { error } = await supabase.from('team_messages').insert({ category, title:title.trim(), message:message.trim(), created_by:currentUser.name, message_date:today, active:true });
    if (error) { setStatus('Message could not be posted.'); return; }
    setTitle(''); setMessage(''); setStatus('Message posted to the team homepage.'); await loadMessages();
  }
  ...
  return <section className="sectionBlock teamMessageBoard">
    <div className="sectionHeading"><div><p className="eyebrow">Daily Team Message</p><h2>Safety, Goals & Important Updates</h2><p className="dashboardRole">Daily messages for the whole team before sign-in.</p></div></div>
    {canPost && <form className="teamMessageForm" onSubmit={postMessage}>
      <div className="requestFormGrid"><label>Category<select value={category} onChange={e=>setCategory(e.target.value as TeamMessage['category'])}><option>Safety</option><option>Goal</option><option>Important</option><option>General</option></select></label><label>Title<input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Example: Today's Safety Focus" /></label></div>
