...
type Machine = {
  id:string; model:string|null; serial_number:string|null; status:string|null;
  started_at:string|null; completed_at:string|null; green_tag_at:string|null;
};
type Request = {
  id:string; machine_id:string|null; requested_at:string; delivered_at:string|null; status:string;
};
...
export default function ProductionIntelligence(){
  ...
  const stats=useMemo(()=>{
    ...
    const linked=requests.filter(r=>r.machine_id);
    const waits=linked.filter(r=>r.delivered_at).map(r=>new Date(r.delivered_at!).getTime()-new Date(r.requested_at).getTime()).filter(v=>v>=0);
  ...
  return <section className="sectionBlock">
    <div className="sectionHeading">
      <div>
        <p className="eyebrow">Production Intelligence · Reconditioning Recovery</p>
        <h2>Turn production data into a catch-up plan</h2>
        <p className="dashboardRole">LineFlow connects machine completion and material-delivery history to show where reconditioning time is being lost and which problems should be addressed first.</p>
      </div>
    </div>
    ...
    <div className="dashboardGrid supervisorCoreGrid">
      ...
      <article className="metricCard"><span>At Risk / Delayed</span><strong>{loading?'—':stats.delayed}</strong><p>Open machines currently marked delayed, on hold, or waiting on material.</p></article>
    </div>

    <div className="dashboardToolPanel" style={{marginTop:12}}>
      <p className="eyebrow">What LineFlow Is Learning</p>
      <h3>Find the bottlenecks preventing us from catching up</h3>
      <p className="dashboardRole">As serial-linked production history grows, LineFlow will identify which machines take longest, which material requests create the most waiting time, recurring shortages, and where the team can recover production time. Predictive recommendations will be added only after enough real production data has been collected.</p>
    </div>

    <div className="dashboardToolPanel" style={{marginTop:12}}>
      <p className="eyebrow">Recovery Focus</p>
      <h3>{stats.backlog===0?'Build the production history':'Reduce the reconditioning backlog'}</h3>
      <p className="dashboardRole">Use this section with the live production schedule to prioritize machines that are ready to finish, expose units blocked by material, and measure whether daily completion pace is improving.</p>
    </div>
  </section>;
}
